...
export default function GRNForm({ mode = "create", grnId }) {
  ...
  const { register, control, reset, getValues, watch, handleSubmit,
    ...
    const fetchGRN = async () => {
      ...
      try {
        ...
        const formData = {
          ...
          partyBillDate:      fmtDate(d.partyBillDate),
  ...
  const handleOrderItemsFetched = (orderData) => {
    if (!orderData) { setItems([]); setInitialItems([]); return; }
  ...
  const buildPayload = () => {
    ...
    if (v.unloadingDatetime)  fd.append("unloadingDatetime",  v.unloadingDatetime.replace("T", " "));
    if (v.physicallyVerifiedBy) fd.append("physicallyVerifiedBy", v.physicallyVerifiedBy);
  ...
  const handleSaveDraft = async () => {
    ...
    let toastId;
    try {
      toastId = toast.loading(mode === "create" ? "Creating GRN..." : "Updating GRN...");
      ...
      toast.success(mode === "create" ? "GRN created successfully" : "GRN updated successfully",
        { id: toastId });
  ...
  const handleSubmitGRN = async () => {
    if (!grnId) { toast.error("Please save as Draft first"); return; }
    let toastId;
    try {
  ...
  const handleFileChange = (e) => {
    ...
    if (!file) { setAttachedFile(null); setNewFileName(""); return; }
    const allowed = [
      "application/pdf",
    ...
    <div className="p-3">
      ...
      <div className="flex flex-col xl:flex-row items-start gap-4">
        ...
        <GRNLeftPanel
          ...
          onVendorClear={() => { setItems([]); setInitialItems([]); }}
        />
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          ...
          <div className="bg-[#d6e6f2] border border-[#7fa3c0] px-3 py-1 text-[13px] font-bold text-[#1a3a5c] rounded-sm">
            BASIC
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-[2px]">
            ...
            <div className="flex items-center gap-[2px]">
              ...
              <Input {...register("deliverVehicleNo")} disabled={disabled}
                placeholder="Text"
            ...
            <div className="flex items-center gap-[2px]">
              ...
              <Input {...register("deliveredConcern")} disabled={disabled}
                placeholder="Text"
                className={`${getInputClass(false, disabled)} flex-1 h-[30px] -ml-px`} />
            </div>
            ...
            <div className="flex items-center gap-[2px]">
              ...
              <Input type="datetime-local" {...register("unloadingDatetime")} disabled={disabled}
                className={`${getInputClass(false, disabled)} flex-1 h-[30px] -ml-px`} />
            </div>
            ...
            <div className="flex items-center gap-[2px]">
              ...
              <Input {...register("physicallyVerifiedBy")} disabled={disabled}
                placeholder="Text"
                className={`${getInputClass(false, disabled)} flex-1 h-[30px] -ml-px`} />
            </div>
          </div>
      ...
      {/* ── ACTION BUTTONS ──────────────────────────────────────────────── */}
      {!isViewMode && (
        <div className="flex justify-end gap-3 mt-5 pt-3 border-t border-[#d8e6f0] flex-wrap">
