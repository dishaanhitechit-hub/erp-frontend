...
import PWOrderItemSelectionModal from "../modals/PWOrderItemSelectionModal";

export default function PWOrderItemsTab({
  form,
  disabled,
  openItemModal,
  setOpenItemModal,
  ...
  // Recalculate amount/gstAmount when rate or gstPercent changes
  const recalculate = (index, field, value) => {
    const qty = Number(items[index]?.qty || 0);
    const rate =
      field === "rate" ? Number(value || 0) : Number(items[index]?.rate || 0);
    const gstPercent =
      field === "gstPercent"
        ? Number(value || 0)
        : Number(items[index]?.gstPercent || 0);
  ...
  return (
    <div className="w-full bg-white">
            ...
            <table className="w-full border-collapse">
              <thead className="sticky top-0 z-10">
                <tr className="bg-[#D3D3D3]">
                  ...
                  {/* Qty is set in modal — disabled in table */}
                  ...
                  {!disabled && (
                    <th className="border px-2 py-1 text-sm min-w-[90px]">Action</th>
                  )}
                </tr>
              </thead>
                          ...
                          render={({ field }) => (
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                recalculate(index, "rate", e.target.value);
                              }}
                            />
                          )}
                        />
                        ...
                        <Controller
                          ...
                          render={({ field }) => (
                            <Input
                              ...
                              onChange={(e) => {
                                field.onChange(e.target.value);
                                recalculate(index, "gstPercent", e.target.value);
                              }}
                            />
                          )}
                        />
      ...
      {/* PW ITEM SELECTION MODAL */}
      <PWOrderItemSelectionModal
        open={openItemModal}
